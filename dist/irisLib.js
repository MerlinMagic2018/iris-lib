(function (global, factory) {
	...
	var util = {
	  getHash: function getHash(str) {
	    ...
	    return gun.SEA.work(str, undefined, undefined, { name: 'SHA-256' });
	  },
	  ...
	  */
	  ...
	  Identity.prototype.identicon = function identicon$$1() {
	    ...
	    async function setIdenticonImg(data) {
	      var hash = await util.getHash(encodeURIComponent(data.type) + ':' + encodeURIComponent(data.value), 'hex');
	...
	*/
	...
	var Index = function () {
	  ...
	  */
	  ...
	  Index.getMsgIndexKey = async function getMsgIndexKey(msg) {
	  ...
	  Index.getMsgIndexKeys = async function getMsgIndexKeys(msg) {
	    var keys = {};
	    var distance = parseInt(msg.distance);
	    distance = _Number$isNaN(distance) ? 99 : distance;
	    distance = ('00' + distance).substring(distance.toString().length); // pad with zeros
	  ...
	  */
	  ...
	  Index.prototype._updateIdentityProfilesByMsg = async function _updateIdentityProfilesByMsg(msg, authorIdentities, recipientIdentities) {
	    var start = void 0;
	    var msgIndexKey = await Index.getMsgIndexKey(msg);
	    ...
	    var _loop2 = async function _loop2() {
	      ...
	      while (author && knownIdentity) {
	        if (author.indexOf(knownIdentity.key) === 0) {
	          try {
	            await util.timeoutPromise(_this9.addMessage(msgsByAuthor[author], { checkIfExists: true }), 10000);
	          } catch (e) {
	  ...
	  */
	  ...
	  Index.prototype.addMessage = async function addMessage(msg) {
	    ...
	    start = new Date();
	    var indexKeys = await Index.getMsgIndexKeys(msg);
	  ...
	  */
	  ...
	  Index.prototype.setReaction = async function setReaction(msg, reaction) {
	    var hash = await msg.getHash();
